...
ROUND(sum(totals_timeonsite) / sum(totals_visits),2) as average_session_duration_secs
from
{{ params.stage_db }}.{{ params.stage_schema }}.{{ params.stage_table_compact }}
...
ROUND(sum(case when totals_bounces = 1 then totals_visits else 0 end)/sum(totals_visits),3) as bounce_rate,
ROUND(SUM(totals_transactions)/COUNT(*),3) AS conversion_rate
FROM
...
AS
SELECT user_action,users,coalesce(ROUND((100-((LAG(CTE.users,1) OVER(order by users DESC))-users)/(LAG(CTE.users,1) OVER(order by users DESC))*100),2),100) AS CONVERSION_PERCENT_RATE_FROM_PREVIOUS_STAGE FROM (SELECT
    ...
    END AS user_action,
    COUNT(fullVisitorID,VISITID) AS users
FROM
    {{ params.stage_db }}.{{ params.stage_schema }}.{{ params.stage_table_compact }} x ,
     lateral flatten(input => x.hits) as hits,
...
--Calculates user's promotion click through rate as per promotion diamensions
